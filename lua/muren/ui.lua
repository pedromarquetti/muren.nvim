...
local update_preview = function()
	...
	if options.values.filetype_in_preview and not options.values.cwd then
		vim.bo[bufs.preview].filetype = vim.bo[options.values.buffer].filetype
	end
...
<<<<<<< HEAD
	...
	if options.values.cwd then
		...
		for i, buf_info in ipairs(buf_info_per_idx) do
			local format_spec =
				format_cwd_preview_line(vim.api.nvim_buf_get_lines(bufs.preview, i - 1, i, true)[1], buf_info)
